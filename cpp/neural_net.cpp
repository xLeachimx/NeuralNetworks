...
NeuralNet::NeuralNet(const NeuralNet &copy){
  ...
  for(int i = 0;i < layers;i++){
    ...
    network[i] = new Neuron[layersSize[i]];
  }
  for(int i = 0;i < layers-1;i++){
    for(int j = 0;j < layerSize[i];j++){
...
vector<double> NeuralNet::run(vector<double> input){
  ...
  if(input.length() != layerSize[0])return vector<double>();
  for(int i = 0;i < layerSize[0];i++){
    network[0][i].input = input[i];
  }
  for(int i = 0;i < layers;i++){
    for(int j = 0;j < layerSize[i];j++){
      network[i][j].activation = activationFunction(network[i][j]);
      for(int k = 0;k < network[i][j].connections.length();k++){
        int to = network[i][j].connections[k].to
        network[i][to].input += network[i][j].connections[k].strength * network[i][j].activation
      }
    }
  }
  vector<double> output = vector<double>();
  for(int i = 0;i < layerSize[layers-1];i++){
    output.push_back(network[layers-1][i]);
  }
  return output;
}
...
void NeuralNet::train(vector<double> input, vector<double> expected, double learningRate){
  ...
  vector<double> diff = vector<double>();
  for(int i = 0;i < output.length();i++){
    diff.push_back(expected[i] - output[i]);
    network[layers-1][i].error = activationDerivative(network[layers-1][i].activation) * diff[i];
  }
